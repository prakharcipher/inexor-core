...
void InexorRenderHandler::Initialize()
{
    ...
    Log.std->debug("InexorRenderHandler initializing...");
    ...
    Log.std->debug("InexorRenderHandler initialized!");
...
bool InexorRenderHandler::GetViewRect(CefRefPtr<CefBrowser> browser, CefRect &rect)
{
    ...
    Log.std->debug("GetViewRect: ({0}x{1} at {2}, {3})", view_width, view_height, view_x, view_y);
    return true;
}
...
{
    if (initialized) {
        // Log.std->info() << "SetViewRect: (" << view_x << ", " << view_y << ", " << view_width << ", " << view_height << ")\n";
        int view_width2 = (view_width / 32) * 32;
        int view_height2 = (view_height / 32) * 32;
        Log.std->debug("SetViewRect: ({0}x{1} at {2}, {3})", view_width2, view_height2, view_x, view_y);
        ...
        Log.std->debug("SetViewRect DEFERRED: ({0}x{1} at {2}, {3})", view_width, view_height, view_x, view_y);
...
void InexorRenderHandler::OnPaint(
    ...
    if (!initialized) {
        if (CefCurrentlyOn(TID_UI)) {
            Log.std->debug("OnPaint:Initialize ({}x{})", width, height);
            Log.std->debug("OnPaint:BeforeInitialization ({0}x{1} at {2}, {3})", view_width, view_height, view_x, view_y);
            Initialize();
            browser->GetHost()->WasResized();
            Log.std->debug("OnPaint:AfterInitialization ({0}x{1} at {2}, {3})", view_width, view_height, view_x, view_y);
            return;
        } else {
            Log.std->debug("InexorRenderHandler::OnPaint() Wrong thread!");
            return;
        }
    }
    ...
    if (type == PET_VIEW) {
        ...
        // view_width = width;
        // view_height = height;
        Log.std->debug("OnPaint:X ({}x{})", width, height);
        ...
        if (!texture_initialized || old_width != view_width || old_height != view_height) {
            ...
            Log.std->debug("OnPaint: Full Resize ({}x{})", width, height);
        ...
        int w = width;
        int h = height;
        Log.std->debug("OnPaint:Y ({}x{})", width, height);

...
void gl_checkextensions()
{
    ...
    {
        hasTF = true;
        if(glversion < 300 && dbgexts) Log.std->debug("Using GL_ARB_texture_float extension.");
        shadowmap = 1;
    ...
    {
        hasTRG = true;
        if(glversion < 300 && dbgexts) Log.std->debug("Using GL_ARB_texture_rg extension.");
    }
    ...
    if(glversion >= 300 || hasext(exts, "GL_ARB_framebuffer_object"))
    {
        ...
        if(glversion < 300 && dbgexts) Log.std->debug("Using GL_ARB_framebuffer_object extension.");
    }
    else if(hasext(exts, "GL_EXT_framebuffer_object"))
    {
        ...
        glGenerateMipmap_          = (PFNGLGENERATEMIPMAPPROC)         getprocaddress("glGenerateMipmapEXT");
        hasFBO = true;
        if(dbgexts) Log.std->debug("Using GL_EXT_framebuffer_object extension.");

        if(hasext(exts, "GL_EXT_framebuffer_blit"))
        {
            glBlitFramebuffer_     = (PFNGLBLITFRAMEBUFFERPROC)        getprocaddress("glBlitFramebufferEXT");
            hasFBB = true;
            if(dbgexts) Log.std->debug("Using GL_EXT_framebuffer_blit extension.");
        }

        if(hasext(exts, "GL_EXT_packed_depth_stencil") || hasext(exts, "GL_NV_packed_depth_stencil"))
        {
            hasDS = true;
            if(dbgexts) Log.std->debug("Using GL_EXT_packed_depth_stencil extension.");
        }
    }
    ...
    if(ati)
    {
        //Log.std->warn() << "WARNING: ATI cards may show garbage in skybox. (use \"/ati_skybox_bug 1\" to fix)";
    ...
    if(glversion >= 300 || hasext(exts, "GL_ARB_map_buffer_range"))
    {
        ...
        hasMBR = true;
        if(glversion < 300 && dbgexts) Log.std->debug("Using GL_ARB_map_buffer_range.");
    }
    ...
    if(glversion >= 310 || hasext(exts, "GL_ARB_uniform_buffer_object"))
    {
        ...
        useubo = 1;
        hasUBO = true;
        if(glversion < 310 && dbgexts) Log.std->debug("Using GL_ARB_uniform_buffer_object extension.");
    }
    ...
    if(glversion >= 300 || hasext(exts, "GL_ARB_vertex_array_object"))
    {
        ...
        hasVAO = true;
        if(glversion < 300 && dbgexts) Log.std->debug("Using GL_ARB_vertex_array_object extension.");
    }
    else if(hasext(exts, "GL_APPLE_vertex_array_object"))
    {
        ...
        hasVAO = true;
        if(dbgexts) Log.std->debug("Using GL_APPLE_vertex_array_object extension.");
    }

    if(glversion >= 330 || hasext(exts, "GL_ARB_texture_swizzle") || hasext(exts, "GL_EXT_texture_swizzle"))
    {
        hasTSW = true;
        if(glversion < 330 && dbgexts) Log.std->debug("Using GL_ARB_texture_swizzle extension.");
    }
...
#endif
        if(dbgexts) Log.std->debug("Using GL_EXT_texture_compression_s3tc extension.");
    }
    ...
    {
        hasS3TC = true;
        if(dbgexts) Log.std->debug("Using GL_EXT_texture_compression_dxt1 extension.");
    }
    if(hasext(exts, "GL_3DFX_texture_compression_FXT1"))
    {
        hasFXT1 = true;
        if(mesa) usetexcompress = max(usetexcompress, 1);
        if(dbgexts) Log.std->debug("Using GL_3DFX_texture_compression_FXT1.");
    }
    ...
    if(hasext(exts, "GL_EXT_texture_filter_anisotropic"))
    {
        ...
        hwmaxaniso = val;
        hasAF = true;
        if(dbgexts) Log.std->debug("Using GL_EXT_texture_filter_anisotropic extension.");
    }
...
void cef_resize(int width, int height)
{
    if (cef_app) {
        ...
        Log.std->info("Update Inexor User Interface Screen Size: {0}x{1}", width, height);
...
void gl_rendercef()
{
    ...
    for(std::list<CefRefPtr<InexorLayer> >::reverse_iterator it = layers.rbegin(); it != layers.rend(); ++it)
    {
        ...
        if (layer.get() && layer->IsVisible()) {
            ...
            if (!initialized) {
                Log.std->debug("err_initialized");
                continue;
            }
            if (view_width == 0 || view_height == 0) {
                Log.std->debug("err_view");
                continue;
            }
            if (texture_id == 0u) {
                Log.std->debug("err_tex");
                continue;
            }

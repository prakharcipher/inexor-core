...
template<class MDL> struct skelcommands : modelcommands<MDL, struct MDL::skelmesh>
{
    ...
    {
        if(!MDL::loading) { Log.std->error("not loading an {0}", MDL::formatname()); return; }
        ...
        if(!mdl.meshes) Log.std->error("could not load {0}", filename);
        else
        {
            mdl.initanimparts();
            mdl.initskins();
    ...
    {
        if(!MDL::loading || MDL::loading->parts.empty()) { Log.std->error("not loading an {0}", MDL::formatname()); return; }
        ...
        Log.std->error("could not find bone {0} for tag {1}", name, tagname);
    }
    ...
    static void setpitch(char *name, float *pitchscale, float *pitchoffset, float *pitchmin, float *pitchmax)
    {
        ...
        if(name[0])
        {
            ...
            Log.std->error("could not find bone {0} to pitch", name);
            return;
        }
    ...
    static void setpitchtarget(char *name, char *animfile, int *frameoffset, float *pitchmin, float *pitchmax)
    {
        ...
        if(!sa) { Log.std->error("could not load {0} anim file {1}", MDL::formatname(), filename); return; }
        skeleton *skel = ((meshgroup *)mdl.meshes)->skel;
        int bone = skel ? skel->findbone(name) : -1;
        if(bone < 0)
        {
            Log.std->error("could not find bone {0} to pitch target", name);
            return;
        }
    ...
    static void setpitchcorrect(char *name, char *targetname, float *scale, float *pitchmin, float *pitchmax)
    {
        ...
        if(bone < 0)
        {
            Log.std->error("could not find bone {0} to pitch correct", name);
            return;
        }
        ...
        if(target < 0)
        {
            Log.std->error("could not find pitch target {0} to pitch correct {1}", targetname, name);
            return;
        }
        pitchcorrect c;
        c.bone = bone;
    ...
    static void setanim(char *anim, char *animfile, float *speed, int *priority, int *startoffset, int *endoffset)
    {
        ...
        if(anims.empty()) Log.std->error("could not find animation {0}", anim);
        else
        {
            ...
            if(!sa) Log.std->error("could not load {0} anim file {1}", MDL::formatname(), filename);
            else loopv(anims)
            {
    ...
    static void setanimpart(char *maskstr)
    {
        ...
        loopv(bonestrs)
        {
            ...
            if(bone<0) { Log.std->error("could not find bone {0} for anim part mask [{1}]", bonestr, maskstr); bonestrs.deletearrays(); return; }
        ...
        if(!p->addanimpart(bonemask.getbuf())) Log.std->error("too many animation parts");
    }
    ...
    static void setadjust(char *name, float *yaw, float *pitch, float *roll, float *tx, float *ty, float *tz)
    {
        ...
        if(!name[0]) return;
        int i = mdl.meshes ? ((meshgroup *)mdl.meshes)->skel->findbone(name) : -1;
        if(i < 0) {  Log.std->error("could not find bone {0} to adjust", name); return; }
